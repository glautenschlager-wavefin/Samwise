...
class MergePRAction(Action):
    """Merge an approved PR once CI is green.

    This is a CONFIRM-level action: it only performs the merge when
    ``settings.auto_merge`` is enabled. Otherwise it emits a notification
    letting the user know the PR is ready to merge.
    """
    ...
    def matches(self, item: ActivityItem) -> bool:
        return (
            "approved" in item.title.lower()
            and bool(item.metadata.get("repo"))
            and bool(item.metadata.get("pr_number"))
        )

    async def execute(self, item: ActivityItem) -> ActivityItem:
        repo = item.metadata["repo"]
        pr_number = int(item.metadata["pr_number"])
        return await self._merge_pr(item, repo, pr_number)
